...
namespace TT
{
    ...
    public class CameraHandler : MonoBehaviour
    {
        ...
        //better to use list, when number of element change
        List<CharacterManager> availableTargets = new List<CharacterManager>(); //all targets that we can lock on to
        ...
        public void HandleCameraRotation(float delta, float mouseXInput, float mouseYInput)
        {
            ...
            else
            {
                ...
                //force rotation to target we are locked on to
        ...
        public void HandleLockOn()
        {
           // availableTargets.Clear();
            ...
            //makes invinsible sphere around player
            Collider[] colliders = Physics.OverlapSphere(targetTransform.position, 50);

            //search all colliders that are detected with overlaps
            for(int i = 0; i < colliders.Length; i++)
            {
                CharacterManager character = colliders[i].GetComponent<CharacterManager>();
                //if characterMagare is found in component
                if (character != null)
                {
